function Build
{
    ...
    $isOnline = Wait-For-Ssh "localhost" $startParams.Port "root" "$($defenition.UsersPassword)"
    if (! $isOnline)
    {
    ...
    # Remote-Command-Raw 'mkdir -p /tmp/build' "localhost" $startParams.Port "root" "$($defenition.UsersPassword)"
    & mkdir -p $mapto/tmp/build
    & cp -a $ProjectPath/lab/* $mapto/tmp/build

    Say "Configure SSH for [$image::$key]"
    Remote-Command-Raw "cd /tmp/build; bash config-ssh.sh $key" "localhost" $startParams.Port "root" "$($defenition.UsersPassword)" $true # re-connect

    Say "RESTART SSH for [$image::$key]"
    Remote-Command-Raw 'sshId="$(pgrep -f "sshd -D")"; Say "Restarting SSH Server (pid is $sshId)"; sudo kill -SIGHUP "$(pgrep -f "sshd -D")"; Say "Restarted SSH Server";' "localhost" $startParams.Port "root" "$($defenition.UsersPassword)" $false $true # destructive

    Say "Configure LC_ALL, UTC and optionally swap for [$image::$key]"
    Remote-Command-Raw "cd /tmp/build; bash config-system.sh $( $definition.SwapMb )" "localhost" $startParams.Port "root" "$($defenition.UsersPassword)" $true # re-connect

    if (!$Is_IgnoreAll)
    {
        Say "Upgrading to the latest Debian for [$image::$key]"
        Remote-Command-Raw 'cd /tmp/build; bash dist-upgrade.sh' "localhost" $startParams.Port "root" "$($defenition.UsersPassword)"

        Say "Installing locales for [$image::$key]"
        Remote-Command-Raw 'cd /tmp/build; bash install-locales.sh' "localhost" $startParams.Port "root" "$($defenition.UsersPassword)"
    }
    ...
    Remote-Command-Raw $cmd "localhost" $startParams.Port "root" "$($defenition.UsersPassword)"
    Remote-Command-Raw $cmd "localhost" $startParams.Port "user" "$($defenition.UsersPassword)"
    ...
    if (!$Is_IgnoreAll)
    {
        ...
        Remote-Command-Raw "$mustHavePackages" "localhost" $startParams.Port "root" "$($defenition.UsersPassword)"
    }

    if ($Is_Requested_Dotnet)
    {
        Say "Installing DotNet Core for [$image::$key]"
        Remote-Command-Raw "cd /tmp/build; bash install-DOTNET.sh; command -v dotnet && dotnet --info || true" "localhost" $startParams.Port "root" "$($defenition.UsersPassword)"
        Remote-Command-Raw 'Say "I am ROOT"; echo PATH is [$PATH]; dotnet --info' "localhost" $startParams.Port "root" "$($defenition.UsersPassword)"
        Remote-Command-Raw 'Say "I am USER"; echo PATH is [$PATH]; dotnet --info' "localhost" $startParams.Port "user" "$($defenition.UsersPassword)"
    ...
    if ($Is_Requested_Mono)
    {
        ...
        Remote-Command-Raw "cd /tmp/build; bash -e install-MONO.sh" "localhost" $startParams.Port "root" "$($defenition.UsersPassword)"
        Remote-Command-Raw 'Say "I am ROOT"; echo PATH is [$PATH]; mono --version; msbuild /version; echo ""; nuget >/tmp/.tmp; cat /tmp/.tmp | head -4; rm /tmp/.tmp' "localhost" $startParams.Port "root" "$($defenition.UsersPassword)"
        Remote-Command-Raw 'Say "I am USER"; echo PATH is [$PATH]; mono --version; msbuild /version; echo ""; nuget >/tmp/.tmp; cat /tmp/.tmp | head -4; rm /tmp/.tmp' "localhost" $startParams.Port "user" "$($defenition.UsersPassword)"
        ...
        Remote-Command-Raw "bash /opt/NET-TEST-RUNNERS/link-unit-test-runners.sh" "localhost" $startParams.Port "root" "$($defenition.UsersPassword)"

        Say "Run .net tests on guest [$image::$key]"
        Remote-Command-Raw "cd /tmp/build; bash -e run-NET-UNIT-TESTS.sh" "localhost" $startParams.Port "root" "$($defenition.UsersPassword)"
    }
    ...
    if ($Is_Requested_Powershell)
    {
        ...
        Remote-Command-Raw "cd /tmp/build; bash -e install-POWERSHELL.sh" "localhost" $startParams.Port "root" "$($defenition.UsersPassword)"
    }

    if ($Is_Requested_Docker)
    {
        Say "Install Docker for [$image::$key]"
        Remote-Command-Raw "cd /tmp/build; bash Install-DOCKER.sh;" "localhost" $startParams.Port "root" "$($defenition.UsersPassword)"
    }

    if ($Is_Requested_local_postgres)
    {
        Say "Install Local Postgres SQL for [$image::$key]"
        Remote-Command-Raw "cd /tmp/build; bash install-POSTGRES.sh;" "localhost" $startParams.Port "root" "$($defenition.UsersPassword)"
    }

    if ($Is_Requested_Local_Mariadb)
    {
        Say "Install Local MariaDB for [$image::$key]"
        Remote-Command-Raw "cd /tmp/build; bash install-MARIADB.sh;" "localhost" $startParams.Port "root" "$($defenition.UsersPassword)"
    }

    if ($Is_Requested_Local_Redis)
    {
        Say "Install Local Redis Server for [$image::$key]"
        Remote-Command-Raw "cd /tmp/build; bash install-REDIS.sh;" "localhost" $startParams.Port "root" "$($defenition.UsersPassword)"
    }
    ...
    if ($Is_Requested_NodeJS)
    {
        ...
        Remote-Command-Raw 'cd /tmp/build; export TRAVIS="$TRAVIS"; bash install-NODE.sh' "localhost" $startParams.Port "root" "$($defenition.UsersPassword)"
        Remote-Command-Raw 'Say "As [$(whoami)] NODE: [$(node --version)]; YARN: [$(yarn --version)]; NPM: [$(npm --version)]"; echo PATH is [$PATH];' "localhost" $startParams.Port "user" "$($defenition.UsersPassword)"
        Remote-Command-Raw 'Say "As [$(whoami)] NODE: [$(node --version)]; YARN: [$(yarn --version)]; NPM: [$(npm --version)]"; echo PATH is [$PATH];' "localhost" $startParams.Port "root" "$($defenition.UsersPassword)"
    }
    <#
    else {
...
#>
    ...
    if (!$Is_IgnoreAll)
    {
        ...
        Remote-Command-Raw "cd /tmp/build; bash install-a-crap.sh" "localhost" $startParams.Port "root" "$($defenition.UsersPassword)"
    }

    Say "Log Packages for [$image::$key]"
    Remote-Command-Raw 'Say "PACKAGES:\n$(list-packages)"' "localhost" $startParams.Port "root" "$($defenition.UsersPassword)"
    ...
    Remote-Command-Raw "cd /; bash /tmp/build/TearDown.sh; apt clean; before-compact" "localhost" $startParams.Port "root" "$($defenition.UsersPassword)" $false $true

    # Say "Dismounting guest's share of [$key]"
    # & umount -f $mapto # NOOOO shutdown?????

    Say "SHUTDOWN [$image::$key] GUEST"
    Remote-Command-Raw "rm -rf /tmp/build; Say 'Size of the /tmp again:'; du /tmp -d 1 -h; sudo shutdown now" "localhost" $startParams.Port "root" "$($defenition.UsersPassword)" $false $true
