...
#>
    ...
    Remote-Command-Raw "rm -rf /tmp/build; Say 'Size of the /tmp again:'; du /tmp -d 1 -h; sync; sudo sync; (sudo shutdown now & ); (poweroff -h now & )" "localhost" $startParams.Port "root" "$($definition.UsersPassword)" $false $true
